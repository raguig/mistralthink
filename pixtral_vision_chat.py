...
def agent_node(state: AgentState):
    ...
    required_tools = list(dict.fromkeys(
        infer_required_tools(latest_query) + infer_required_tools_from_plan(plan)
    ))
...
def tools_node(state: AgentState):
    ...
    for tool_call in tool_calls:
        result = execute_tool_by_name_and_args(
            tool_call.get("name", ""),
...
def critic_node(state: AgentState):
    ...
    required_tools = list(dict.fromkeys(
        infer_required_tools(latest_query) + infer_required_tools_from_plan(state.get("plan", ""))
    ))
        ...
        if retry_count >= max_retries:
            return {
                "messages": [
                    AIMessage(
                        content=(
                            f"{last_answer}\n\nStopped after retry limit. "
                            f"Could not complete required tools: {', '.join(missing_tools)}."
                        )
                    )
                ],
                "needs_retry": False,
        ...
        if retry_count >= max_retries:
            return {
                "messages": [
                    AIMessage(
                        content=(
                            f"{last_answer}\n\nStopped after retry limit while revising the answer. "
                            f"Last critique was:\n{critique}"
                        )
                    )
                ],
